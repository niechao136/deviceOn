...
function main({text, device, content, type, question, api, token, timezone}) {
  // 处理 LLM 结果
  ...
  const {
    request,
    filter,
  } = filterDevice(JSON.parse(device), obj, type, content, api, token, question, timezone, 'remote_desktop')
  ...
  const task = filter.length > 1 ? 'remote_desktop' : ''
