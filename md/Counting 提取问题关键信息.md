...
## 地点分析
请根据{query}文本内容，确定涉及地点特征(`site`、`province`、`city`、`country`)，若无匹配则为空数组
- 在{query}文本中**精确匹配** `{{#1745891380321.site_name#}}` 内的任一元素，将所有匹配项存入 `site`
- 在{query}文本中**精确匹配** `{{#1745891380321.province#}}` 内的任一元素，将所有匹配项存入 `province`
- 在{query}文本中**精确匹配** `{{#1745891380321.city#}}` 内的任一元素，将所有匹配项存入 `city`
- 在{query}文本中**精确匹配** `{{#1745891380321.country#}}` 内的任一元素，将所有匹配项存入 `country`
...
## 5. 地点识别（字段 `site`、`province`、`city`、`country`）

- 请**精确匹配** query 中是否包含以下门店名列表（`{{#1745891380321.site_name#}}`）：
    - 若匹配，将所有匹配项加入 `site` 数组
    - 若无匹配，返回 `site = []`
- 请**精确匹配** query 中是否包含以下区域一列表（`{{#1745891380321.province#}}`）：
    - 若匹配，将所有匹配项加入 `province` 数组
    - 若无匹配，返回 `province = []`
- 请**精确匹配** query 中是否包含以下区域二列表（`{{#1745891380321.city#}}`）：
    - 若匹配，将所有匹配项加入 `city` 数组
    - 若无匹配，返回 `city = []`
- 请**精确匹配** query 中是否包含以下国家列表（`{{#1745891380321.country#}}`）：
    - 若匹配，将所有匹配项加入 `country` 数组
    - 若无匹配，返回 `country = []`

并说明地点相关4个栏位判断依据 `site_reason`
